...
TEXT _start(SB), 1, $-4
	/*
	 * code0 must be a raw WORD, not "B _hdrend(SB)": the 7l
	 * follow() pass chases real branch instructions and lays
	 * the target code at offset 0, scattering the header words.
	 * 0x14000010 = b .+64 (branch over the 64-byte header).
	 */
	WORD	$0x14000010		/* code0: b .+64 */
	WORD	$0xD503201F		/* code1: nop */
	WORD	$0x00100000		/* text_offset (lo) */
	WORD	$0			/* text_offset (hi) */
	WORD	$0x02000000		/* image_size = 32MB (lo) */
	WORD	$0			/* image_size (hi) */
	WORD	$0x00000002		/* flags: LE, 4K, near RAM base */
	WORD	$0			/* flags (hi) */
	WORD	$0
	WORD	$0			/* res2 */
	WORD	$0
...
_zerobss:
	...
	/*
	 * No SEVL/WFE parking: with PSCI CPU_ON, secondaries are
	 * only started (by mpinit) after cpu0 has built the page
	 * tables, so there is nothing to wait for.  Under VZ a
	 * WFE genuinely waits, and nobody ever SEVs.
	 */
_startup:
	BL	mmuenable<>(SB)
...
TEXT cmpswap(SB), 1, $-4
	/*
	 * Use ARMv8.1 CAS instead of LDXR/STXR to avoid
	 * exclusive monitor livelock under Apple VZ.
	 *
	 * CASW R1, R2, [R0]
	 *   old = *R0; if old==R1 then *R0 = R2; R1 = old
	 */
	MOVWU	ov+8(FP), R1
	MOVWU	nv+16(FP), R2
	MOVWU	R1, R3		/* save expected value */
	WORD	$0x88A17C02	/* CASW R1, R2, [R0] */
	DMB	$ISH
	CMPW	R1, R3		/* R1 = old; did it match? */
	BNE	_cas0
	MOVW	$1, R0
	RETURN
_cas0:
...
TEXT _tas(SB), 1, $-4
	/*
	 * Apple Virtualization.framework clears the cpu exclusive
	 * monitor on every guest exit (timer, host interrupt, etc.),
	 * so LDXR/STXR loops can livelock.  Use an ARMv8.1 atomic
	 * swap instead.  All Apple Silicon supports LSE atomics.
	 *
	 * SWPW R1, R0, [R0]  --  old = *R0; *R0 = R1; R0 = old
	 */
	MOVW	$0xdeaddead, R1
	WORD	$0xB8218000
...
TEXT lcycles(SB), 1, $-4
	/* no PMU under Apple VZ; use the virtual counter */
	MRS	CNTVCT_EL0, R0
	RETURN
...
/*
 * Reload m (R27) and up (R26) from their canonical locations.
 * Call after returning from libc code that may clobber them.
 */
TEXT restoreregs(SB), 1, $-4
	MRS	TPIDR_EL1, R27
	MOV	$0, R26
	RETURN
