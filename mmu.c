...
l1map(uintptr va, uintptr pa, uintptr pe, uintptr attr)
{
	...
	while(pa < pe){
		...
		if(l1[PTL1X(va, 1)] & PTEVALID) {
			if((l1[PTL1X(va, 1)] & PTETABLE) != PTETABLE){
				/*
				 * Already mapped as a large block (e.g. by mmu0init).
				 * Skip to the next block-sized boundary.
				 */
				va += PGLSZ(1) - (va & (PGLSZ(1)-1));
				pa += PGLSZ(1) - (pa & (PGLSZ(1)-1));
				continue;
			}
